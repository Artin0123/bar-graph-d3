...
function setupCanvas(barChartData) {
    ...
    const chart_margin = { top: 80, left: 80, right: 40, bottom: 40 };
    ...
    const xMin = d3.max(barChartData, d => d.revenue);
    ...
    const bars = this_svg.selectAll('.bar').data(barChartData).enter().append('rect').attr('class', 'bar').attr('x', 0).attr('y', d => yScale(d.genre)).attr('width', d => xScale_v2(d.revenue)).attr('height', yScale.bandwidth()).style('fill', 'dodgerblue');
